...
import type { SmsnaCategoryKey } from "@/lib/smsna/taxonomy";
import { getOprsInstrument, OPRS_CITATION, OPRS_LIKERT, OPRS_DIFFICULTY } from "@/lib/smsna/oprs";
...
// OPRS accent color — distinct from EpaObservationForm's EPA stage-color
// palette so an OPRS form is visually recognizable as "not an EPA".
const OPRS_COLOR = "#a855f7";

// ── Shared styles (theme-aware via CSS variables) — copied from
// EpaObservationForm.tsx to match its visual language exactly. These are
// not exported there, so they are duplicated here rather than imported. ──
...
export function OprsObservationForm({
  categoryKey, categoryName, procedureName,
  prefillData, onSubmit, onCancel, onSaveDraft,
}: OprsObservationFormProps) {
  const instrument = getOprsInstrument(categoryKey);
  ...
  // 3-point OPRS case-difficulty scale — rated first, per the OPRS contract.
  const [difficulty, setDifficulty] = useState<1 | 2 | 3 | null>(null);

  // Procedure-specific + general items, both on the 5-point OPRS Likert scale.
  const [specificRatings, setSpecificRatings] = useState<CriterionRating[]>(
    () => instrument.specific.map((item) => ({
      criterionId: item.id,
      label: item.label,
  ...
  const [generalRatings, setGeneralRatings] = useState<CriterionRating[]>(
    () => instrument.general.map((item) => ({
      criterionId: item.id,
      label: item.label,
      entrustmentRating: null,
      comment: "",
  ...
  // Overall item (OV1) — OPRS 5-point Likert, NOT the Royal College O-Score.
  // OPRS's competence anchor is 3 ("Competent"), not 4.
  ...
  // (copied verbatim from EpaObservationForm.tsx)
  ...
  function updateSpecificRating(index: number, rating: number | null) {
    setSpecificRatings((prev) => prev.map((cr, i) => (i === index ? { ...cr, entrustmentRating: rating } : cr)));
  }
  function updateSpecificComment(index: number, comment: string) {
    setSpecificRatings((prev) => prev.map((cr, i) => (i === index ? { ...cr, comment } : cr)));
  }
  function updateGeneralRating(index: number, rating: number | null) {
    setGeneralRatings((prev) => prev.map((cr, i) => (i === index ? { ...cr, entrustmentRating: rating } : cr)));
  }
  function updateGeneralComment(index: number, comment: string) {
    setGeneralRatings((prev) => prev.map((cr, i) => (i === index ? { ...cr, comment } : cr)));
  }

  function handleOverallRating(score: number) {
    setOverallRating(score);
    // OPRS competence anchor is 3 ("Competent"), not 4 like the Royal
    // College O-Score used elsewhere in this app.
    setAchievement(score >= 3 ? "ACHIEVED" : "NOT_ACHIEVED");
  }
  ...
  function buildInput(): EpaObservationInput {
    ...
    const difficultyLabel = OPRS_DIFFICULTY.find((d) => d.value === difficulty)?.label;

    return {
      caseLogId: prefillData?.caseLogId,
      epaId: `SMSNA-${categoryKey.toUpperCase()}`,
      epaTitle: `OPRS: ${categoryName}`,
      specialtySlug: "smsna",
      trainingSystem: "SMSNA",
      observationDate: new Date(observationDate),
      setting: basisOfAssessment,
      complexity: difficulty ? `Difficulty ${difficulty}: ${difficultyLabel}` : undefined,
      technique: procedureName || undefined,
      assessorName, assessorRole,
      assessorEmail: assessorEmail || undefined,
      achievement: achievement || undefined,
      // On SMSNA rows, entrustmentScore carries the OPRS 5-point Likert
      // rating for the overall item — not the Royal College O-Score.
      entrustmentScore: overallRating ?? undefined,
      observationNotes: commentsOptional || undefined,
      strengthsNotes,
      improvementNotes,
      // On SMSNA rows, each entrustmentRating below is an OPRS 5-point
      // Likert rating, not the Royal College O-Score.
      criteriaRatings: [...specificRatings, ...generalRatings],
      safetyConcern,
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 700 }}>
      ...
      {/* ── Header ──────────────────────────────────────────────── */}
      <div style={{
        padding: "16px 18px", background: `${OPRS_COLOR}06`,
        border: `1px solid ${OPRS_COLOR}20`, borderRadius: 12,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          <span style={{
            fontSize: 14, fontWeight: 700, color: OPRS_COLOR,
            fontFamily: "'Geist Mono', monospace",
            background: `${OPRS_COLOR}15`, padding: "3px 10px", borderRadius: 6,
          }}>
            OPRS
          </span>
          <span style={{ fontSize: 9, fontWeight: 600, color: OPRS_COLOR, textTransform: "uppercase", letterSpacing: ".5px", opacity: 0.7 }}>
            SMSNA Fellowship
          </span>
        </div>
        <div style={{ fontSize: 16, fontWeight: 600, color: "var(--text)", lineHeight: 1.4, marginBottom: 4 }}>
          {categoryName}
        </div>
      ...
      {/* ── Difficulty — rated first, per the OPRS contract ──────── */}
      <div style={sectionStyle}>
        <h3 style={sectionTitleStyle}>Case Difficulty</h3>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
          {OPRS_DIFFICULTY.map((d) => {
            const selected = difficulty === d.value;
            const color = d.value === 1 ? "#10b981" : d.value === 2 ? "#f59e0b" : "#ef4444";
            return (
              <button
                key={d.value}
                type="button"
                ...
                style={{
                  ...
                  border: selected ? `2px solid ${color}` : "1px solid var(--border-mid)",
                  background: selected ? `${color}10` : "var(--bg-3)",
                  cursor: "pointer", transition: "all .15s", textAlign: "center",
                }}
              >
                <div style={{ fontSize: 13, fontWeight: selected ? 700 : 500, color: selected ? color : "var(--text-2)" }}>
                  {d.label}
                </div>
              </button>
            );
      ...
      <div style={sectionStyle}>
        ...
        {/* ── Send for sign-off — picker block (copied from EpaObservationForm) ── */}
        <div style={{
          marginTop: 16, padding: "16px",
          background: "var(--bg-2)", border: "1px solid var(--border-mid)", borderRadius: 12,
        }}>
            ...
            <div>
              {pickedAssessor ? (
                <div style={{
                  ...
                  background: "rgba(16,185,129,.08)", border: "1px solid rgba(16,185,129,.3)", borderRadius: 8,
                }}>
                  <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: "1px solid var(--border-mid)", borderRadius: 6, cursor: "pointer",
                    }}
                  >
                    Change
                  </button>
                ...
                <>
                    ...
                    {pickerResults.map(a => (
                      <button
                        ...
                        style={{
                          ...
                          textAlign: "left", cursor: "pointer", marginBottom: 6, fontFamily: "inherit",
                        }}
                      >
                        <div style={{
                  ...
                  <div style={{ fontSize: 11, color: "var(--text-3)", marginTop: 6 }}>
                    They&rsquo;ll get an in-app notification in their Sign-Offs inbox (with email backup).
                  </div>
                </>
              )}
            </div>
                ...
                <div style={{
                  ...
                  {lookupLoading
                    ...
                    : assessorLookup?.isHippoUser
                      ? <><span style={{ color: "#10b981", fontWeight: 700 }}>✓</span>{" "}This email matches a Hippo user — we&rsquo;ll route as in-app + email backup.</>
                      : <>We&rsquo;ll email a secure sign-off link valid for 12 months. No login needed.</>
                  }
                </div>
              )}
            </div>
      ...
      {/* ── Procedure-specific items (5-point OPRS Likert) ───────── */}
      <OprsItemGroup
        title={`${categoryName}: Procedure-Specific Items`}
        ratings={specificRatings}
        onRate={updateSpecificRating}
        onComment={updateSpecificComment}
      />

      {/* ── General items (5-point OPRS Likert, every instrument) ── */}
      <OprsItemGroup
        title="General Items"
        ratings={generalRatings}
        onRate={updateGeneralRating}
        onComment={updateGeneralComment}
      />

      {/* ── Overall item ──────────────────────────────────────────── */}
      <div style={sectionStyle}>
        <h3 style={sectionTitleStyle}>{instrument.overall.label}</h3>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 6, marginBottom: 16 }}>
          {OPRS_LIKERT.map((opt) => {
            const selected = overallRating === opt.value;
            return (
              <button
                key={opt.value}
                type="button"
                onClick={() => handleOverallRating(opt.value)}
                style={{
        ...
        {overallRating !== null && (
          <div style={{
            ...
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: achievement === "ACHIEVED" ? "#10b981" : "#64748b" }} />
            <span style={{ fontSize: 12, fontWeight: 600, color: achievement === "ACHIEVED" ? "#10b981" : "#94a3b8" }}>
              {achievement === "ACHIEVED"
                ? "Achieved: competence threshold met (OPRS ≥ 3)"
                : "Not Yet Achieved: further development needed (OPRS < 3)"}
            </span>
          </div>
        )}
      ...
      {/* ── Continue, Change, Consider (copied from EpaObservationForm) ── */}
      <div style={sectionStyle}>
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ ...labelStyle, color: "#10b981" }}>
            Continue — What should the fellow keep doing?
          </label>
          <textarea
            value={continueNotes}
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ ...labelStyle, color: "#f59e0b" }}>
            Change — What should the fellow do differently?
          </label>
          <textarea
            value={changeNotes}
        ...
        <div>
          <label style={{ ...labelStyle, color: "#0ea5e9" }}>
            Consider — What should the fellow think about?
          </label>
          <textarea
            value={considerNotes}
      ...
      {/* ── Safety / Professionalism Flags (copied from EpaObservationForm) ── */}
      <div style={{
        ...sectionStyle,
      ...
      {/* ── Footer (copied from EpaObservationForm) ──────────────── */}
      <div style={{ display: "flex", alignItems: "center", gap: 10, paddingTop: 4 }}>
        ...
        <button type="submit" disabled={submitting || !assessorName.trim()}
          style={{
            ...
            background: OPRS_COLOR, color: "#fff", fontSize: 13, fontWeight: 600,
...
// ── Per-item OPRS Likert rating group — mirrors the CanMEDS criteria
// table in EpaObservationForm.tsx (a 6-column, 0-5 grid), adapted to the
// 5-column, 1-5 OPRS scale. ──
function OprsItemGroup({
  title, ratings, onRate, onComment,
}: {
  title: string;
  ...
  return (
    <div style={sectionStyle}>
      ...
      {/* Column headers */}
      <div style={{
        display: "grid", gridTemplateColumns: "1fr repeat(5, 52px)", gap: 0,
        fontSize: 10, fontWeight: 700, color: "#cbd5e1", textAlign: "center",
        padding: "6px 0 10px", borderBottom: "2px solid var(--border-mid)", marginBottom: 2,
      }}>
        <div style={{ textAlign: "left", fontSize: 11, color: "#94a3b8" }}>Item</div>
        {OPRS_LIKERT.map((opt) => (
      ...
      {ratings.map((cr, idx) => (
        <div key={cr.criterionId}>
          <div style={{
            display: "grid", gridTemplateColumns: "1fr repeat(5, 52px)", gap: 0, alignItems: "center",
            padding: "12px 6px", borderBottom: "1px solid var(--border-mid)",
            background: idx % 2 === 0 ? "transparent" : "rgba(255,255,255,0.03)", borderRadius: 6,
          }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", lineHeight: 1.4, paddingRight: 12 }}>
              {cr.label}
            </div>
            {OPRS_LIKERT.map((opt) => {
              ...
              return (
                <div key={opt.value} style={{ display: "flex", justifyContent: "center" }}>
                  <button
                    type="button"
                    onClick={() => onRate(idx, selected ? null : opt.value)}
                    style={{
...
// ── ChoiceCard — copied from EpaObservationForm.tsx (private/unexported there). ──
function ChoiceCard({
